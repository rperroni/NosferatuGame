...
int ** CostMatrices::getHumanCosts() {
    return this -> humanCost -> getMatrix();
}
...
void CostMatrices::show_matrixs() {

    cout << "MATRIZ HUMANOS" << endl;

    for(int i = 0; i < vertexAmount; i++){
        for(int j = 0; j < vertexAmount; j++){

            cout << humanCost -> getData(i, j) << " ";
        }

        cout << "" << endl;
    }

    cout << "MATRIZ CAZADOR" << endl;

    for(int i = 0; i < vertexAmount; i++){
        for(int j = 0; j < vertexAmount; j++){

            cout << hunterCosts -> getData(i, j) << " ";
        }

        cout << endl;
    }

    cout << "MATRIZ VAMPIRO" << endl;

    for(int i = 0; i < vertexAmount; i++){
        for(int j = 0; j < vertexAmount; j++){

            cout << vampireCosts -> getData(i, j) << " ";
        }

        cout << endl;
    }

    cout << "MATRIZ ZOMBIE" << endl;

    for(int i = 0; i < vertexAmount; i++){
        for(int j = 0; j < vertexAmount; j++){

            cout << zombieCosts -> getData(i, j) << " ";
        }
        cout << endl;
    }
}
